...
class MaggamCatalogSerializer(serializers.HyperlinkedModelSerializer):
    ...
    def create(self, validated_data):
        ...
        try:
            ...
            if self.initial_data.get("catalog_relations"):
                if self.initial_data.get("catalog_relations").get("images"):
    ...
    def update(self, instance, validated_data):
        try:
            maggam_catalog, created = MaggamCatalog.objects.update_or_create(**validated_data)
            if created:
                print("Maggam catelog update")
                maggam_catalog.save()
            if self.initial_data.get("images"):
                for e in maggam_catalog.images.all():
                    maggam_catalog.images.remove(e)
